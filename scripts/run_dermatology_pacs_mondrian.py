...
for rep in range(a.reps):
 for model_idx,e in enumerate(preds):
  ...
  for mq in (.65,.75,.85):
   ...
   methods.append((f'pacs_mondrian_q{mq:.2f}',pacs.predict(e[te])))
...
df=pd.DataFrame(rows); tag=str(q).replace('.','p'); df.to_csv(out/f'dermatology_pacs_mondrian_q{tag}_raw.csv',index=False)
s=df.groupby('method').agg(success=('case_mass_success','mean'),success_sd=('case_mass_success','std'),mean_size=('mean_size','mean'),mean_size_sd=('mean_size','std'),p90_size=('p90_size','mean'),mean_mass=('mean_mass','mean'),minority=('minority_fraction','mean'),runs=('rep','count')).reset_index(); s.to_csv(out/f'dermatology_pacs_mondrian_q{tag}_summary.csv',index=False); print(s.to_string(index=False,float_format=lambda x:f'{x:.4f}'))
