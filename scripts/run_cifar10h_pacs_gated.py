...
def metrics(mask, lam, q):
    ...
    size = mask.sum(1)
    modal = np.argmax(lam, 1)
    minority = mass - np.max(lam, axis=1) * mask[np.arange(len(mask)), modal]
    return dict(
        success=float(np.mean(mass >= q - 1e-12)),
        mean_size=float(np.mean(size)),
        p90_size=float(np.quantile(size, .9)),
        p95_size=float(np.quantile(size, .95)),
        mean_mass=float(np.mean(mass)),
        minority=float(np.mean(minority)),
    )
...
def main():
    ...
    ap.add_argument('--q', type=float, default=.7)
    args = ap.parse_args()

    lam = normalize_probs(np.load(HUM))
    e = normalize_probs(np.load(PRED))
    assert e.shape == lam.shape == (10000, 10)
    q = float(args.q)
    alpha = .1
    rows = []
    tag = str(q).replace('.', 'p')
    ...
    for rep in range(args.reps):
        rng = np.random.default_rng(2026082600 + 1000 * rep + int(q * 100))
        p = rng.permutation(len(e))
        tr, cal, te = p[:4000], p[4000:7000], p[7000:]
        ...
        methods.append(('global_panel', predict_global(e[te], th), {}))

        gated = DisagreementGatedPACS(
            q_mass=q,
            alpha_case=alpha,
            alpha_gate=alpha,
            random_state=20260826 + rep,
            gate_fraction=.5,
            model_quantile=.9,
            n_bins=4,
            min_cal_per_bin=100,
        ...
        for method, mask, diag in methods:
            ...
            r.update(method=method, rep=rep, q=q)
            for key, val in diag.items():
                r[f'gate_{key}'] = val
            rows.append(r)
        pd.DataFrame(rows).to_csv(OUT / f'cifar10h_pacs_gated_q{tag}_raw.csv', index=False)
    ...
    summary = df.groupby('method').agg(
        ...
        p95_size=('p95_size', 'mean'),
        mean_mass=('mean_mass', 'mean'),
        minority=('minority', 'mean'),
        runs=('rep', 'count'),
    ).reset_index()
    summary.to_csv(OUT / f'cifar10h_pacs_gated_q{tag}_summary.csv', index=False)
