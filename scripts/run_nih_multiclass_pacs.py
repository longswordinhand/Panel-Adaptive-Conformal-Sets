...
def main():
    ...
    for rep in range(args.reps):
        ...
        pacs=PACSTopKCV(q,alpha,random_state=20260817+rep).fit(etr,lam[tr],ecal,lam[cal]); methods.append(('pacs_cv',pacs.predict(ete)))
        for method,mask in methods:
            r=metrics(mask,lam[te],q); r.update(method=method,rep=rep,q=q,ntrain=len(tr),ncal=len(cal),ntest=len(te),selected_q=(pacs.selected_quantile_ if method=='pacs_cv' else np.nan)); rows.append(r)
        print('done',rep,'n',len(tr),len(cal),len(te),'pacsq',pacs.selected_quantile_,flush=True)
    df=pd.DataFrame(rows); tag=str(q).replace('.','p'); df.to_csv(OUT/f'nih_multiclass_pacs_q{tag}_raw.csv',index=False)
    s=df.groupby('method').agg(success=('success','mean'),success_sd=('success','std'),mean_size=('mean_size','mean'),mean_size_sd=('mean_size','std'),p90_size=('p90_size','mean'),mean_mass=('mean_mass','mean'),support=('support','mean'),runs=('rep','count')).reset_index(); s.to_csv(OUT/f'nih_multiclass_pacs_q{tag}_summary.csv',index=False)
