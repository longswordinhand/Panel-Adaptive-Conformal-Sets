...
from src.pacs_v3 import PACSNormalizedTopK
...
for model_idx in range(4):
 ...
 methods=[('global',predict_global(e[te],th)),('topk_v2',PACSTopK(q,.1,0).fit(e[tr],lam[tr],e[cal],lam[cal]).predict(e[te])),('norm_topk_v3',PACSNormalizedTopK(q,.1,0).fit(e[tr],lam[tr],e[cal],lam[cal]).predict(e[te]))]
...
out=Path('experiments/pr_rescue/results'); df.to_csv(out/f'dermatology_pacs_v3_q{q:.1f}_raw.csv',index=False); s.to_csv(out/f'dermatology_pacs_v3_q{q:.1f}_summary.csv',index=False)
