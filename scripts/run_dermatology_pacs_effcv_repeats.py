...
for rep in range(a.reps):
 for model_idx,e in enumerate(preds):
  ...
  pacs=PACSTopKEffCV(q,.1,random_state=rep*101+model_idx).fit(e[tr],lam[tr],e[cal],lam[cal]); methods.append(('pacs_effcv',pacs.predict(e[te]),pacs.selected_quantile_))
