...
ap=argparse.ArgumentParser(); ap.add_argument('--model',type=int,required=True); a=ap.parse_args(); model_idx=a.model
...
df=pd.DataFrame(rows); print(df[['method','case_mass_success','mean_size','p90_size','mean_mass','minority_fraction']].to_string(index=False,float_format=lambda x:f'{x:.4f}'))
out=Path('experiments/pr_rescue/results'); df.to_csv(out/f'topk_quantile_q09_model{model_idx}.csv',index=False)
